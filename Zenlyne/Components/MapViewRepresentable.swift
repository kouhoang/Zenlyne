...
public struct MapViewRepresentable: UIViewRepresentable {
    ...
    public class Coordinator: NSObject {
        ...
        func createFriendMarkerImage(for mapView: MapView, friendId: String? = nil, name: String? = nil) {
            let size: CGFloat = 50 // Square dimensions
            let renderer = UIGraphicsImageRenderer(size: CGSize(width: size, height: size))
            ...
            let annotationImage = renderer.image { ctx in
                ...
                // Create gradient background - use different color for friends
                let colors = [
                    UIColor(red: 1.0, green: 0.5, blue: 0.0, alpha: 0.9).cgColor, // Orange for friends
            ...
            // Generate a unique ID for each friend marker
        ...
        func updateFriendAnnotations(for mapView: MapView, friendLocations: [String: UserLocation], friends: [User]) {
            ...
            for (friendId, location) in friendLocations {
                ...
                createFriendMarkerImage(for: mapView, friendId: friendId, name: friend.fullName)
                
                // Create annotation
                var annotation = PointAnnotation(coordinate: location.toCoordinate())
                annotation.iconAnchor = .bottom
                annotation.iconImage = "friend-marker-\(friendId)"
                annotation.iconSize = 1.0
                annotation.textField = friend.fullName
